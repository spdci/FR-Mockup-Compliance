@method=POST @endpoint=fr/sync/search
...
It will be called by the SP systems or other registries.
    ...
    Scenario: Successfully search FR to be processed
        Given System wants to sync search for person in FR
    ...
    Scenario: Validate the structure of reg_records in search response Functional Test
        Given System has sent a sync search request for FR person for a functional test
