...
Given(/^SP has previously sent an unsubscribe request to FR$/, function () {
  specUnsubscribe = spec(); // Initialize the specSearch object
});

When(/^FR completes processing and calls SP on-unsubscribe callback$/, async function () {
  try {
...
Then(/^SP should receive the on-unsubscribe response from FR$/, async function () {
