...
Given(/^SP has previously sent a txn status request to FR$/, function () {
  spectxn = spec(); // Initialize the spectxn object
});

When(/^FR completes processing and calls SP txn on-status callback$/, async function () {
  try {
...
Then(/^SP should receive the txn on-status response from FR$/, async function () {
